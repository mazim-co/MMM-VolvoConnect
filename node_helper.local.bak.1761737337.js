...
_setupAuthRoutes() {
  ...
  const generatePKCE = () => {
    ...
    const challenge = require("crypto")
      ...
      .update(random)
      .digest("base64url");
    return { verifier: random, challenge };
  };
  ...
  app.get(`${base}/login`, (req, res) => {
    ...
    console.log("[VolvoConnect] redirecting to", authUrl);
  ...
  app.get(`${base}/callback`, async (req, res) => {
    ...
    try {
      ...
      console.error("[VolvoConnect] PKCE callback error:", e.response?.status, e.message);
